import { motion } from "framer-motion";
import { MouseEvent, useState } from "react";
import { useAppSelector } from "src/app/hooks";
import { selectTheme, Theme } from "src/features/theme/themeSlice";

import "./ButtonWithIcon.css";
import LoaderSimple from "../../loader/LoaderSimple";

export interface ButtonWithIconProps {
    icon: string,
    onClickAction: (event: MouseEvent<HTMLButtonElement, globalThis.MouseEvent>) => void,
    keyValue?: string,
    initialStyle?: any,
    exitStyle?: any,
    defaultStyle?: any,
    defaultIconStyle?: any,
    onHoverStyle?: any,
    onHoverIconStyle?: any,
    onClickStyle?: any,
    onClickIconStyle?: any,
    className?: string | null,
    isLoading?: boolean,
    text?: string | null
    type?: "button" | "submit" | "reset" | undefined
}

export default function ButtonWithIcon(props: ButtonWithIconProps) {
    const theme: Theme = useAppSelector(selectTheme);
    const [isHover, setHover] = useState<boolean>(false);
    const [isClick, setClick] = useState<boolean>(false);
    return (
        <motion.button
            type={props.type != null ? props.type : "button"}
            key={props.keyValue}
            className={"ButtonWithIcon " + (props.className ? props.className : "") + (props.text != null ? " text" : "")}
            onClick={(event) => props.onClickAction(event)}
            onMouseEnter={() => setHover(true)}
            onMouseLeave={() => setHover(false)}
            onMouseDown={() => setClick(true)}
            onMouseUp={() => setClick(false)}
            whileHover={{
                scale: 1.1,
                boxShadow: "0 0 10px " + theme.colors.shadowMain,
                ...props.onHoverStyle
            }}
            whileTap={{
                scale: 0.9,
                ...props.onClickStyle
            }}
            initial={{
                background: theme.colors.button.withIcon.background,
                borderColor: theme.colors.border,
                ...props.defaultStyle,
                ...props.initialStyle
            }}
            animate={{
                background: theme.colors.button.withIcon.background,
                borderColor: theme.colors.border,
                ...props.defaultStyle
            }}
            exit={{
                ...props.exitStyle
            }}
            layout>

            {
                props.isLoading ? <LoaderSimple color={"#ffffff"}/> : (
                    <>
                    <motion.img
                        src={props.icon}
                        style={{...props.defaultIconStyle}}
                        animate={isClick ? props.onClickIconStyle : isHover ? props.onHoverIconStyle : {}}
                        layout/>
                    {props.text != null && <motion.span animate={{color: theme.colors.textMain}}>{props.text}</motion.span>}
                    </>
                )
            }
        </motion.button>
    )
}
